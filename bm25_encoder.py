...
from base_sparse_encoder import BaseSparseEncoder
...
class BM25Encoder(BaseSparseEncoder):
    ...
    def fit(self, corpus: List[str]) -> "BM25Encoder":
        ...
        for doc in tqdm(corpus, desc="Fitting BM25"):
        ...
        self.n_docs = n_docs
        self.avgdl = sum_doc_len / n_docs if n_docs > 0 else 0.0
        return self
    ...
    def _encode_single_document(self, text: str) -> SparseVector:
        ...
        tf = np.array(doc_tf, dtype=np.float64)
        tf_sum = sum(tf)
    ...
    def _encode_single_query(self, text: str) -> SparseVector:
        ...
        df = np.array([self.doc_freq.get(idx, 1) for idx in indices], dtype=np.float64)
        idf = np.log((self.n_docs + 1) / (df + 0.5))

        # Avoid division by zero
        idf_sum = idf.sum()
        if idf_sum == 0:
            idf_norm = np.zeros_like(idf)
        else:
            idf_norm = idf / idf_sum
    ...
    def set_params(
            ...
            avgdl: float,
            n_docs: int,
            doc_freq: Dict[str, List[Union[int, float]]],
            b: float,
            k1: float,
            lower_case: bool,
        ...
        self.doc_freq = {
            int(idx): float(val)
            for idx, val in zip(doc_freq["indices"], doc_freq["values"])
        }
        self.b = b
        self.k1 = k1
        self._tokenizer = BM25Tokenizer(
    ...
    def default() -> "BM25Encoder":
        ...
        try:
            ...
            raise RuntimeError(f"Failed to load default MS MARCO model: {str(e)}")
